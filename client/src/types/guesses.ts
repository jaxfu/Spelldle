...
export type T_GUESS_STATES_DISPLAY = string | string[];
export type T_GUESS_STATES_IDS = number | number[];

export type T_GUESS_MAP_IDS = Map<string, T_GUESS_STATES_IDS>;
...
export function translateIdsToDisplay(
	...
	switch (categoryInfo.component_type) {
		...
		case E_CATEGORY_COMPONENT_TYPE.LEVEL:
			if (Array.isArray(id)) {
				const display: T_GUESS_STATES_DISPLAY = [categoryInfo.values[id[0]]];
